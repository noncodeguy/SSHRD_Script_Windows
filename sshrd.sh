...
elif [ "$oscheck" = 'MINGW64_NT-10.0-19045' ]; then
    if ! (powershell -Command "Get-PnpDevice -PresentOnly | Where-Object { \$_.FriendlyName -like '*Apple Mobile Device (DFU Mode)*' }" >> /dev/null); then
        echo "[*] Waiting for device in DFU mode"
    fi
    
    while ! (powershell -Command "Get-PnpDevice -PresentOnly | Where-Object { \$_.FriendlyName -like '*Apple Mobile Device (DFU Mode)*' }" >> /dev/null); do
        sleep 1
    done
else
    echo "[-] Unsupported OS"
...
while true; do
    if "$oscheck"/irecovery -q | grep -q "MODE: DFU"; then
        break
    else
        echo "[*] Please enter DFU mode and press Enter to continue"
        read -r 
    fi
done
...
echo "TEST if this works irecovery is fine"
...
echo "TEST if this works rm & mkdir is fine"
...
echo "TEST if this works darwin_major is fine"
...
if [ $oscheck = 'MINGW64_NT-10.0-19045' ]; then
    echo "[*] Do you want to pwn manually with WinRa1n? (y/n)"
    read answer
    while true; do
        if [ "$answer" = "y" ]; then
            echo "[*] Please pwn the device with WinRa1n and then press enter"
            read
        else
            echo "[*] You're not advised to pwn with gaster. Make sure you installed libusbK via Zadig!"
            "$oscheck"/gaster pwn 
        fi
        isitpwned=$("$oscheck"/irecovery -q | grep PWND)
        if [ "$isitpwned" != "PWND: checkm8" ] && [ "$isitpwned" != "PWND: IKEYM8" ]; then
            echo "[-] Device is not pwned. Please pwn the device and try again"
            exit
        else
            break
        fi
...
echo "TEST if this works img4tool is fine"
...
cd ..
echo "TEST if this works pzb is fine"
...
echo "TEST if this works decrypt is fine"
